...
# asc = atan2( sinθ*cosε - tanφ*sinε, cosθ )
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# ✅ Planet positions tropical (NASA/Skyfield) with speed
# ---------------------------------------------------------
def get_planets_ecliptic(datetime_utc_iso: str, lat: float, lng: float):
    """
    Returns:
      jd_ut (float)
      tropical planet list (lon/lat/dist/speed)

    NOTE: lat/lng not used here (geocentric observer), kept for signature compatibility.
    """
    _ensure_loaded()
    ...
    # speed approx: (lon(t+1min) - lon(t-1min))/2min * 1440min/day
    ...
    for disp, key in PLANETS:
        ...
        results.append(
            {"name": disp, "lon": lon, "lat": lat_e, "dist_au": dist, "speed_lon": speed_lon}
        )

    jd_ut = float(t.ut1)  # UT-like JD, good for astrology + swe.get_ayanamsa_ut
    return jd_ut, results


# ---------------------------------------------------------
# ✅ Helper: Convert tropical -> sidereal using exact ayanamsa
# ---------------------------------------------------------
def tropical_to_sidereal_lon(trop_lon_deg: float, jd_ut: float, mode: str = "KP") -> float:
    """
    mode: "KP" default, or "LAHIRI"
    """
    ay = get_ayanamsa_deg(jd_ut, mode)
    return _wrap360(trop_lon_deg - ay)
